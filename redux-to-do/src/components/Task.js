    ...
    return (
        <div className='task'>
            <div className='taskDescription'>
                <input type="checkbox" checked={isChecked} onClick={() => dispatch(updateExistingTaskCompletion(props.id))}/>
                <span>{task.description}</span>
            </div>
            {task.displayUpdate === true && <UpdateTask id={props.id}/>}
            <div className='taskActions'>
                <button className='update' onClick={() => dispatch(showHideTaskUpdateForm(props.id))}>
